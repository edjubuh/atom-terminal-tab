...
export default class TerminalSession {
  ...
  handleEvents() {
    ...
    this.pty.on('exit', () => {
      const linger = atom.config.get('terminal-tab-plus.linger') || 0;
      if (linger >= 0) {
        setTimeout(this.destroy.bind(this), linger);
      }
    });
  ...
  get shellPath() {
    ...
    return this._shellPath = this.config.shellPath
      || atom.config.get('terminal-tab-plus.shellSettings.shellPath')
      || process.env.SHELL
      || process.env.COMSPEC;
  }
  ...
  get shellArguments() {
    ...
    return this._shellArguments = this.config.shellArgs
      || atom.config.get('terminal-tab-plus.shellSettings.shellArgs')
      || '';
  }
  ...
  get sanitizedEnvironmentKeys() {
    ...
    return this._sanitizedEnvironmentKeys = this.config.sanitizeEnvironment
      || atom.config.get('terminal-tab-plus.shellSettings.sanitizeEnvironment');
  }
  ...
  getDefaultLocation() {
    return atom.config.get('terminal-tab-plus.defaultLocation');
  }
